			...
			foreach (ChangingOffset city in citys) {
				...
				if (city.gameObject.tag == backgroundTagString) {
					...
					_cityView.Background [0].items [selectedBackgroundItem].stingItemOffset = 
						string.Format ((int)Camera.main.WorldToScreenPoint(city.transform.position).x + 
							" " + (int)Camera.main.WorldToScreenPoint(city.transform.position).y);
					...
					_cityView.Construction [selectedConstruction].left = (int)Camera.main.WorldToScreenPoint(city.transform.position).x;
					_cityView.Construction [selectedConstruction].top = (int)Camera.main.WorldToScreenPoint(city.transform.position).y;
